...
export const creditService = {
  ...
  async getTransactions(
    ...
    try {
      const { data } = await api.get<ApiResponse<CreditTransaction[]>>(
  ...
  async initiatePayment(
    packageIndex: number,
    provider?: string
  ): Promise<{ payment: Payment; checkoutUrl?: string; package: CreditPackage }> {
    try {
      const { data } = await api.post<
        ApiResponse<{ payment: Payment; checkoutUrl?: string; package: CreditPackage }>
      >('/payments', { packageIndex, provider });
      return data.data!;
    } catch (error) {
      handleApiError(error);
    }
  ...
  async confirmPayment(
    paymentId: string,
    transactionData?: { mpPaymentId?: string; mpStatus?: string }
  ): Promise<Payment> {
    try {
