...
import { StudyDocument, ApiResponse } from '../types';
...
export const documentService = {
  ...
  async list(page = 1, limit = 20): Promise<{ documents: StudyDocument[]; total: number; pages: number }> {
    try {
      const { data } = await api.get<ApiResponse<StudyDocument[]>>('/documents', {
        params: { page, limit },
      });
      return {
        documents: data.data!,
